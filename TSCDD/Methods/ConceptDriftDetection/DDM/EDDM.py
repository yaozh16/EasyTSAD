...
import numpy as np
from river.drift.binary import EDDM as river_eddm


class EDDM(BaseOnlineMethod4CD):

    def __init__(self, hparam: dict = None):
        super().__init__(hparams=hparam)
        warm_start = self.hparams.get("warm_start", 3)
        alpha = self.hparams.get("alpha", 0.95)
        beta = self.hparams.get("beta", 0.9)
        ocs_params = self.hparams.get("ocs_params", dict())
        self.classifier = OneClassSVM(**ocs_params)
        self.model = river_eddm(warm_start, alpha=alpha, beta=beta)
    ...
    def online_collect_results(self) -> MethodTestResults:
        ...
        label_store.set_label(ReportPointLabel.from_point_list(report_point_indexes, len(indexes), "EDDM(RP)"))
        ...
        label_store.set_label(RunLengthLabel(run_length_values, "EDDM(RL)"))

        change_point_indexes = np.array((report_point_indexes - run_lengths) // 1, dtype=np.int32)

        label_store.set_label(ChangePointLabel.from_point_list(change_point_indexes, len(indexes), "EDDM(CP)"))
