...
class Detector:
    ...
    def __init__(self, data, model_universe, model_prior, cp_model,
        ...
        *data*. E.g., if you have a SxS spatial lattice with T time points,
        ...
        if data is not None:
            ...
            self.data = np.zeros(shape=(T, S1 * S2))
        ...
        if step_size_param_learning is None:
            self.step_size_param_learning = (Detector.default_step_size_param_learning)
    ...
    def run(self, start=None, stop=None):
        ...
        for t in range(start - 1, stop - 1):
            if t % self.notifications == 0 and time2 != 0.0:
                ...
                print("Last iteration took " + str(time.process_time() - time2) +
                      " seconds")
            time2 = time.process_time()
            self.next_run(self.data[t, :], t + 1)
        self.execution_time = time.process_time() - time_start

    def next_run(self, y, t: int):
        ...
        assert isinstance(y, np.ndarray) and y.shape[1] == self.S1 * self.S2

        self.test_possible_extend_storage_according_to_t_y(t, y)

        if self.max_lag_length + 3 < t and self.alpha_param_opt_t < t:
            # only do if MAP has changed last iteration!
            if t > 3:
                if self.CPs[t - 2][-1][0] != self.CPs[t - 3][-1][0]:  # 如果过去2个lag的时间的run length不变
    ...
    def update_all_joint_log_probabilities(self, y, t):
        ...
        if (self.generalized_bayes_rld == "power_divergence"
            ...
            at_least_one_model_initialized = np.any([(model.has_lags and (t - model.lag_length) > 1) or
                                                     (not model.has_lags)
                                                     for model in self.model_universe])
            ...
            if at_least_one_model_initialized:
                ...
                for m, model in zip(range(0, self.Q), self.model_universe):
                    ...
                    if warmed_up:
                        ...
                        all_log_alpha_derivatives_sign[
                            m,
                            model_indices_indicators_relative_to_all_run_lengths
                        ] = model.log_alpha_derivatives_joint_probabilities_sign
                ...
                model_sums_derivatives, model_sums_derivatives_sign = logsumexp(
                    a=all_log_alpha_derivatives,
                    b=all_log_alpha_derivatives_sign,
                    return_sign=True,
                    axis=0
                )
                model_sums = logsumexp(
                    a=all_log_probs,
                    axis=0
                )
                ...
                expr, sign = logsumexp(
                    ...
                    b=np.array([sign_1,
                                sign_2 * np.ones(self.Q)[:, np.newaxis]]),
                    return_sign=True,
                    axis=0
                )
    ...
    def MAP_estimate(self, t):
        ...
        *log_MAP_proposed* quantity implied by that model will beat the current 
        ...
        self.log_MAP_storage = np.append(self.log_MAP_storage,
                                         log_MAP_current)  # [t-1] = log_MAP_current #problem log map storage
    ...
    def test_possible_extend_storage_according_to_t_y(self, t: int, y):
        if t < self.T:
            self.data[t] = y
            return
        self.data = np.vstack([self.data, np.zeros(shape=(self.T, self.S1 * self.S2))])
        self.storage_mean = np.vstack([self.storage_mean, np.zeros(shape=(self.T, self.S1, self.S2))])
        self.storage_var = np.vstack([self.storage_var, np.zeros(shape=(self.T, self.S1 * self.S2))])
        self.storage_log_evidence = np.hstack([self.storage_log_evidence, -np.inf * np.ones(shape=self.T)])
        self.CPs.extend([[]] * self.T)
        self.T *= 2
        self.data[t] = y
